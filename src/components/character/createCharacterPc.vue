<template>
  <div class="box-container-main row justify-center" style="height: 100vh">
    ...
    <div class="col-4 self-center" align="center">
      <div class="q-pa-md">
        <q-img
          style="max-width: 320px; width: 60%"
          src="/images/character/label-create-character-pc.png"
        ></q-img>
      </div>
                ...
                <div align="center" class="full-width">
                  <span class="f16">
                    {{
                      ` ${
                        i == 1
                          ? "ทรงผม " + equipment.head.substr(equipment.head.length - 1)
                          : ""
                      }
                              ${
                                i == 2
                                  ? "เสื้อ " +
                                    equipment.body.substr(equipment.body.length - 1)
                                  : ""
                              }
                              ${
                                i == 3
                                  ? "กางเกง " +
                                    equipment.footer.substr(equipment.footer.length - 1)
                                  : ""
                              }
                            `
                    }}</span
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const checkName = () => {
      let regex = /\s/g;
      let found = characterName.value.match(regex);
