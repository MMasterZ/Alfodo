<template>
  <q-dialog v-model="isShowDialogFinishPractice" persistent maximized class="z-max">
        ...
        <div class="box-dialog-container relative-position">
          ...
          <div class="absolute-top box-star-container">
            <div class="relative-position">
              <div class="absolute-top box-star-main" :class="i == 1 ? 'left' : i == 2 ? 'center' : 'right'"
                align="center" v-for="i in 3">
                <div class="relative-position animate__animated animate__fadeIn" :style="`animation-delay:${i * 0.1}s`">
                  <div class="relative-position animate__animated animate__fadeIn">
                    <q-img :src="`/images/box_main/box-icon-practice-star-${i == 1 ? 'left' : i == 2 ? 'center' : 'right'
                      }-blank.webp`" no-spinner no-transition></q-img>
                  </div>
                  <div class="absolute-center fit" v-if="isShowAnimationStar && practiceData.star >= i">
                    <div class="relative-position animation-star" :class="i == 1 ? 'left' : i == 2 ? 'center' : 'right'"
                      :style="`animation-delay:${i * 0.1}s`">
                      <q-img :src="`/images/box_main/box-icon-practice-star-${i == 1 ? 'left' : i == 2 ? 'center' : 'right'
                        }.webp`" no-spinner no-transition></q-img>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="sub-dialog" align="center">
            ...
            <div class="relative-position box-text-finish font-luckiest-guy">
              <div class="absolute-center full-width row justify-center">
                <div class="relative-position animation-show-text" v-for="(itemText, indexText) in showTextFinish"
                  :key="indexText" :style="`animation-delay:${indexText * 0.15}s;
                `">
                  <div class="relative-position animate__animated" :style="`animation-delay:${indexText * 0.15
                    }s;animation-duration:0.5s`" :class="`animate__${showAniamtionFinish[indexText]}`">
                    <div class="label-shadow-finish">
                      <div v-html="itemText == '-' ? '&nbsp;' : itemText"></div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <!-- #region reward -->
            <div class="content-dialog row justify-center relative-position">
              <div class="absolute-top box-label-reward font-fredoka f12" align="center">
                <div class="sub-label-reward">REWARD</div>
              </div>
              <!-- #region ตั๋วเล่นเกม -->
              <div class="self-end box-ticket relative-position" v-if="
                (practiceData.isFirstTimeReward && practiceData.star > 0) ||
                (!practiceData.isFirstTimeReward &&
                  practiceStore.practiceCurrentCounter > 0)
              ">
                <div class="absolute-center" style="top: 40%">
                  <q-img src="/images/icon_main/icon-game-ticket.webp" no-spinner no-transition
                    width="28.939px"></q-img>
                </div>
              ...
              <!-- #region exp -->
              <div class="col box-exp f24 relative-position font-mali-b row justify-center"
                :class="practiceData.isFirstTimeReward ? 'q-mt-md' : ''">
                <div class="absolute-top box-bonus-frist-time" v-if="practiceData.isFirstTimeReward">
                  <div class="relative-position">
                    <q-img src="/images/box_main/box-bonus-first-time.webp" no-spinner no-transition></q-img>
                    ...
                    <div class="absolute-top-left animation-frame">
                      <sprite-animation :setWidth="320" :setHeight="320"
                        :setImage="`/images/sprite_main/sprite-frame-first-time.png`" :setActive="{
                          frameRate: 20,
                          duration: 0,
                          isHover: false,
                          isActive: true,
                ...
                <div class="col-12 self-end row justify-center items-center" style="margin-bottom: 0px">
                  <div class="self-center q-mt-xs">
                    {{ `${setAmount(totalExp)}` }}
                  </div>
                  <div class="self-end q-ml-sm">
                    <q-img src="/images/icon_main/icon-text-xp.webp" no-spinner no-transition width="38px"></q-img>
                  </div>
                </div>
              </div>
              <!-- #endregion -->

              <div align="center" class="box-text-ticket col-12 f10 font-mali-sb"
                v-if="practiceData.star > 0 && studentStore.studentData.gameTicket >= 5">
                {{ `Play Ticket ถึงจำนวนจำกัดแล้ว` }}
              </div>
            </div>
            <!-- #endregion -->

            <!-- #region แสดงจำนวนครั้งที่เหลือ -->
            <div class="box-times-left row justify-center" v-if="
              !synchronizeStore.isSync && !practiceStore.currentPracticeData.isLesson
            ">
              <div class="f12 font-mali-b self-center">
                {{ `การบันทึกคะแนน :` }}
              </div>
              <div class="box-count-number row justify-center items-center f10 font-mali-b">
                <div>
                  {{ `เหลือ` }}
                </div>
                <div class="relative-position" style="margin: 0px 4px"
                  :class="practiceStore.practiceCurrentCounter >= 2 ? 'text-finish' : ''">
                  {{
                    ` ${practiceStore.practiceCurrentCounter == 0
                      ? "2"
            ...
            <!-- #region โหมดควบคุม -->
            <div v-if="synchronizeStore.isSync" class="control-dialog font-mali-sb f10" align="center">
          ...
          <div class="absolute-bottom footer-dialog row justify-center" v-if="!synchronizeStore.isSync">
            ...
            <div class="q-mx-xs">
              <q-img width="76px" class="button-active-main" :class="practiceStore.practiceCurrentCounter > 0 && practiceStore.nextPractice
                ? ''
                : 'disable'
                " :src="`/images/button_main/button-next-to-practice${practiceStore.practiceCurrentCounter > 0 && practiceStore.nextPractice
                  ? ''
                  : '-disable'
          ...
          <div class="absolute-top box-label-front">
            <q-img :src="`/images/label_main/label-${practiceData.star > 0 ? 'complete-practice' : 'try-again'
              }-front.webp`" no-spinner no-transition></q-img>
          </div>

          <div class="absolute-top box-label-back">
            <q-img :src="`/images/label_main/label-${practiceData.star > 0 ? 'complete-practice' : 'try-again'
              }-back.webp`" no-spinner no-transition></q-img>
          </div>
        </div>
      </q-card-section>
    </q-card>
...
// Boolean
const isShowDialogFinishPractice = ref(true);
const isShowAnimationStar = ref(false);
...
const funcCloseDialog = () => {
  isShowDialogFinishPractice.value = false;
...
const funcRestartPractice = () => {
  ...
  // practiceStore.setPracticeData(props.practiceData);
...
const funcSplitText = () => {
  // showCounter.value -= props.practiceData.counter;

  let tempText =
    wordingScore.value[props.practiceData.star][Math.floor(Math.random() * 3)];

  tempText.toLocaleUpperCase();

  let data = tempText.split("");
  ...
  setTimeout(() => {
    isShowAnimationStar.value = true;
  }, 1000);
};
...
const totalExp = computed(() => {
  ...
  if (props.practiceData.isFirstTimeReward) {
    exp = props.practiceData.totalCorrectQuestion * 1500;
  } else {
    exp = props.practiceData.totalCorrectQuestion;
  }
