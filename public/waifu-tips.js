...
const live2d_settings = {
  ...
  // model
  'modelName': 'winny',                      // default model name when first visit website
  ...
  // style
  // 'live2dHeight': '100%',                        // height of Live2D model, NO 'px' in the end
  // 'live2dWidth': `100%`,                         // width of Live2D model, NO 'px' in the end
  'waifuMinWidth': '0',                  // hide model when window width less than setting, eg, '1040px' (Recommend) or 'disable'
  'waifuEdgeSide': '',                 // position of model, eg, 'left:0' or 'right:30', can be override by model setting
  // misc
  ...
  'homePageUrl': '',  // homepage, could be URL or 'auto'
  'aboutPageUrl': '', // about page
  'screenshotCaptureName': '',   // filename of screenshot, eg, 'live2d.png'
}
// 模型列表
const live2d_models = [
  {
...
//   if (state.systemData.isChangeAIModel) {
//     loadOtherModel()
//   }
// })
...
//   live2d_settings.live2dWidth = canvasContainer.clientWidth;
//   canvas.height = canvasContainer.clientHeight;

//   // Add your drawing code here
//   // For example, you can draw a red rectangle:
...
function initModel() {
  ...
  if (live2d_settings.preLoadMotion) {
    window.live2dv4.setPreLoadMotion(live2d_settings.preLoadMotion);
  }

  if (live2d_settings.debug) {
    window.live2dv2.debug = true;
    window.live2dv4.debug = true;
  }
  else {
    window.live2dv2.debug = false;
    window.live2dv4.debug = false;
  }


  if (live2d_settings.debugMousemove && live2d_settings.debug) {
    window.live2dv2.debugMousemove = true
    window.live2dv4.debugMousemove = true
  } else {
    window.live2dv2.debugMousemove = false
    window.live2dv4.debugMousemove = false
  }
...
function loadModel(modelName) {
  ...
  // console.log('systemStore.systemData.isChangeAIModel: ', systemStore.systemData.isChangeAIModel);

  // systemStore.systemData.isChangeAIModel = false;

  // console.log('modelName: ', modelName);
  // console.log('live2d_models: ', live2d_models);
  ...
  // console.log('window.live2dCurrentVersion: ', window.live2dCurrentVersion);
  // console.log('window.live2dv2.: ', window.live2dv2);

  window.live2dv4.release();

  $$(`#${live2dId4}`).style.display = 'none';

  // if (systemStore.systemData.isChangeAIModel) {
  //   window.live2dv4.change(`model/winny`, `${modelName}.model3.json`);
  // } else {

  // }

  $$(`#${live2dId4}`).style.display = 'block';
  window.live2dv4.load(live2dId4, `model/winny`, `${modelName}.model3.json`);
